    ...
    <div className="prose prose-invert max-w-none prose-sm">
      <ReactMarkdown
        ...
        components={{
          ...
          h2: ({ node, ...props }) => (
            <h2 className="text-xl font-bold text-gray-100 mb-2 mt-3" {...props} />
          ),
          h3: ({ node, ...props }) => (
            <h3 className="text-lg font-semibold text-gray-200 mb-2 mt-2" {...props} />
          ),
          ...
          // Lists
          ul: ({ node, ...props }) => (
            <ul className="list-disc list-inside text-gray-300 mb-2 space-y-1 ml-2" {...props} />
          ),
          ol: ({ node, ...props }) => (
            <ol className="list-decimal list-inside text-gray-300 mb-2 space-y-1 ml-2" {...props} />
          ),
          li: ({ node, ...props }) => (
          ...
          code: ({ node, inline, className, ...props }: any) => {
            ...
            return (
              <code className="block bg-gray-900 text-gray-100 p-3 rounded-lg overflow-x-auto mb-2 font-mono text-sm" {...props} />
            );
          },
          pre: ({ node, ...props }) => (
            <pre className="bg-gray-900 text-gray-100 p-3 rounded-lg overflow-x-auto mb-2 font-mono text-sm" {...props} />
          ),

          // Blockquotes
          blockquote: ({ node, ...props }) => (
            <blockquote className="border-l-4 border-blue-500 pl-4 italic text-gray-400 my-2 py-1" {...props} />
          ),
          ...
          th: ({ node, ...props }) => (
            <th className="border border-gray-700 px-3 py-2 text-left text-gray-100 font-semibold" {...props} />
          ),
          td: ({ node, ...props }) => (
